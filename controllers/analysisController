...
class AnalysisController {
  ...
  async processVideo(req, res) {
    ...
    // FIXED: Initialize analysisState outside try block to fix scope issue
    let analysisState = {
      videoId: videoId,
      status: 'processing',
    ...
    try {
      ...
      res.json({
        ...
        processingTime: processingStats.total_time,
        result: completeAnalysis,
        message: 'Tactical analysis completed successfully'
      });
      ...
      res.status(500).json({
        ...
        status: 'failed',
        error: error.message,
        stage: analysisState.currentStage || 'unknown'
      });
    }
  }
  
  /**
   * Quick analysis for urgent tactical needs
   */
